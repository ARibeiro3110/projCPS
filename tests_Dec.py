...
print("Error Decryption 1:", scheme.CalculateDecError(sk, C1, mu=mu1))

mu2 = 1
C2 = scheme.Enc(pk, mu=mu2)
print("\nmu2:", mu2)
print("Decrypted 2:", scheme.Dec(sk, C2))
print("Error Decryption 2:", scheme.CalculateDecError(sk, C2, mu=mu2))

for i in range(5):
    print("\n##### Iteration:", f"{i+1}", "#####")

    mu = int(not(mu1 and mu2))
    print("mu:", mu)

    C = scheme.NAND(C1, C2)
    print("Decrypted NAND:", scheme.Dec(sk, C))

    e = scheme.CalculateDecError(sk, C, mu=mu)
